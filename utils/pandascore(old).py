...
async def fetch_all_tournaments():
    """
    Загружает все турниры и фильтрует только нужные поля (id, name, league_id, tier, status).
...
async def fetch_all_matches():
    """
    Загружает все матчи и фильтрует только нужные поля (id, tournament_id, status, opponents, begin_at, winner_id, streams).
    Возвращает список словарей.
    """
    matches = []
    page = 1
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        while True:
            url = f"{BASE_URL}/matches?page={page}&per_page={per_page}"
            r = await client.get(url, headers=HEADERS)
            if r.status_code != 200:
                break
            data = r.json()
